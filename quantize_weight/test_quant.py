...
# def to print output of first layer
...
def main():
    ...
    modle_fuse = torch.quantization.fuse_modules(model_quant, fuse_list, inplace=True)
    modle_fuse.qconfig = torch.quantization.QConfig(
    ...
    print("QConfig:", modle_fuse.qconfig)
    model_prepared = torch.quantization.prepare(modle_fuse, inplace=True)
    model_convert = torch.quantization.convert(model_prepared, inplace=True)
    # Load trọng số đã quantized
    # model_quant.load_state_dict(torch.load("detect_quant_weight.pth", map_location=device))
    model_convert.load_state_dict(torch.load("quantize_weight/complex.pth", map_location=device, weights_only=False))
    ...
    if haha == 0:
        ...
        with torch.no_grad():
            print_first_layer_output(model_convert, input_tensor)
            outputs = model_convert(input_tensor)
